...
| What | Qual fato ou transição de estado ocorreu?   O que exatamente ocorreu? | message, event_type | DSL / Código de Negócio |
...
| Why | Qual foi a intenção ou motivação?  Qual a motivação ou causa de negócio?  | reason, error_code | Lógica de Domínio |
| How | Por qual canal o evento foi disparado?  Por qual canal chegou? | transport, channel | Camada de Entrada (REST, MQ) |
...
Em arquiteturas de microsserviços, o *Who* inclui também o serviço que gerou o evento. O campo `negotiation-module` ou `business module`  é preenchido automaticamente a partir de `quarkus.application.name` (Quarkus) ou da configuração equivalente no container Jakarta EE:

```json
{
  "business-module": "pedidos-service"
}
```
...
O What é o identificador do evento. Para ser útil, ele deve ser determinístico.
